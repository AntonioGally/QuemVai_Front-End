  ...
  const handleClick = async () => {
    try {
      ...
      if (response.status === 400) {
        setErros("Houve algum problema ao cancelar a solicitação");
      }
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <div>
      <Modal size="sm" centered show={show} onHide={onHide}>
        ...
        <Modal.Body>
          <p>
            Certeza que você deseja cancelar a solicitação de amizade para o
            usuário <b className="text-danger">{id} ?</b>
          </p>

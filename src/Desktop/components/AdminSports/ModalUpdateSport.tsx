...
const AdminSports: React.FC<Props> = ({ show, onHide, id }) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const [SportInfo] = responses;
      const Sport = await SportInfo.data;
      setData({ SportInfo: Sport });
    });
  }, [id]);
  ...
  const onSubmitForm = async (data: FormCadastroAlter) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setErros("");
        setSuccess("Esporte atualizado com sucesso!");
      }
      if (response.status !== 200) {
                    ...
                    <Form.Group style={{ margin: 0, width: "100%" }}>
                      <MyInput>
                        <Form.Control
                          name="nameSport"
                          id="nameSport"
                          defaultValue={data?.SportInfo.created.name}
                          ref={register({
                            maxLength: {
                              value: 30,
                    ...
                    <Form.Group style={{ margin: 0, width: "100%" }}>
                      <MyInput>
                        <Form.Control
                          as="textarea"
                          rows={8}
                          defaultValue={data?.SportInfo.created.description}

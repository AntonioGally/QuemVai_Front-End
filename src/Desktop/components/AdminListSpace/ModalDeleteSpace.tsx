...
const AdminListSpace: React.FC<Props> = ({ show, onHide, id }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await api.delete(`/api/admin/find/delete/${id}`, config);
  ...
  return (
    <Modal size="sm" animation={true} centered show={show} onHide={onHide}>
      <Modal.Header closeButton>
        <Modal.Title className="text-danger">Deletar espaço</Modal.Title>
      </Modal.Header>

      <Modal.Body>
        <p>Tem certeza que deseja deletar o espaço selecionado?</p>
        <Row>

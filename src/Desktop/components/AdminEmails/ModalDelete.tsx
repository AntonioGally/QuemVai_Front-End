...
const AdminEmails: React.FC<Props> = ({ show, onHide, id }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await api.delete(`/api/admin/delete/email/${id}`, config);

      if (response.status === 200) {
        setErros("");
        setSuccess("Email deletado com sucesso");
      }
      if (response.status !== 200) {
  ...
  return (
    <Modal size="sm" animation={true} centered show={show} onHide={onHide}>
      <Modal.Header closeButton>
        <Modal.Title className="text-danger">Deletar email</Modal.Title>
      </Modal.Header>

      <Modal.Body>
        <p>Tem certeza que deseja deletar o email selecionado?</p>
        <Row>
